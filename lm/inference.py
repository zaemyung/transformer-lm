...
class ModelWrapper:
    ...
    def get_log_probs(self, tokens: List[str], past: List[torch.Tensor]=None) -> Tuple[torch.Tensor, List[torch.Tensor]]:
        ...
        with torch.no_grad():
            # print ("past: %s", past)
            res = self.model(ctx, past)
            logits = res['logits'].squeeze(0)
            return torch.log_softmax(logits, dim=1), res['presents']
